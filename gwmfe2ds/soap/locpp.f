...
*     Check for positive triangle areas.
      errc = 0
C      do 20 j=1,n
C       if(axy(j) .le. 0.) then
C       trix = (x0(j) + x1(j) + x2(j))/3.
C       triy = (y0(j) + y1(j) + y2(j))/3.
C       errc = j
C       return
C       endif
C   20 continue
...
C*      Normal derivatives to the edges.
C       dudx = -ayu(j)/axy(j)
C       dudy = -aux(j)/axy(j)
C       dudn0(j) = (dy0(j)*dudx - dx0(j)*dudy)/l0(j)
C       dudn1(j) = (dy1(j)*dudx - dx1(j)*dudy)/l1(j)
C       dudn2(j) = (dy2(j)*dudx - dx2(j)*dudy)/l2(j)
