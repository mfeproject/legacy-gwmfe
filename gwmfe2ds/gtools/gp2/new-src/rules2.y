...
%token  PLOT XSECT OPEN CLOSE NO DRAW GRID SHOW TIMES DOT NFILE VECT LEQ
	...
	CONT CSPACE OFFSET SHEAR SIZE FORMAT ZRANGE QXS
...
plotcom : PLOT		{sprintf(fseq,"%d",pic->framenumber);
	...
	| PLOT frameseq {writepicspec(pic,fseq,&plspec,qxs);}
...
etcetera: DRAW		{linecount = 0;pflag = 1;openpl();erase();drawscreen(pic,mfp,qxs);}
...
xsectcom: xinit			{initxsect(0,0,pic);}
	...
	| ZRANGE number ':' number {setzrange($2,$4,pic);}
	| ZRANGE '*'    {pic->xspec[pic->zindex].usersetzrng = 0;}
	;
xinit	: XSECT		{qxs = FALSE;}
	| QXS		{qxs = TRUE;}
	;
